...
export const extensions = [
  {
    ...
    keywords: ["write", "text", "header", "title"]
...
export const Command = ({ extensions, setter, select, selected }) => {
  ...
  return (
    <div
      ...
      {extensions.length === 0 && (
        <div className="flex text-zinc-500 bg-transparent h-8 items-center text-xs">No results</div>
      )}
      {extensions.length > 0 && (
        <div
          ...
          {extensions.map((ex: any, i: number) => (
            <div
              ...
              key={`extension_${i}`}
              className={`text-xs flex justify-between text-sm h-8 p-2 py-2 rounded-md ${selected === i ? "bg-zinc-200/50 active" : 'bg-transparent'}`}
              onClick={() => setter(ex)}>
